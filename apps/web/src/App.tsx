...
function SessionUnavailable({ refetch }: { refetch: () => Promise<void> }) {
  ...
  return (
    <div className="grid h-full place-items-center bg-[var(--rk-n101)] px-6 text-center">
      <div className="flex flex-col items-center">
        <LoadingState label={t`Reconnecting`} />
        <p className="mt-3 text-[13.5px] text-[var(--rk-n35)]">
...
function ShellSkeleton() {
  return (
    <div
      className="flex h-full overflow-hidden bg-[var(--rk-n101)]"
      data-rakazo-app-state="session-pending"
    >
      <aside className="hidden w-[316px] shrink-0 border-e border-[var(--rk-n84)] bg-[var(--rk-n98)] px-3.5 pt-16 md:block">
        <div className="h-10 rounded-xl bg-[var(--rk-n89)]" />
        <div className="mt-5 space-y-2 px-1">
          {[0, 1, 2, 3].map((row) => (
            <div key={row} className="flex items-center gap-3 rounded-xl px-2 py-2.5">
              <div className="h-9 w-9 rounded-full bg-[var(--rk-n82)]" />
              <div className="flex-1 space-y-2">
                <div className="h-3 w-2/5 rounded bg-[var(--rk-n69)]" />
                <div className="h-2.5 w-4/5 rounded bg-[var(--rk-n87)]" />
              </div>
            </div>
          ))}
        </div>
      ...
      <main className="flex flex-1 flex-col">
        <div className="h-[74px] border-b border-[var(--rk-n89)]" />
        <div className="flex flex-1 items-center justify-center text-[14px] text-[var(--rk-n40)]">
          <Trans>Opening your Space…</Trans>
        </div>
        <div className="mx-6 mb-6 h-[54px] rounded-full border border-[var(--rk-n70)] bg-[var(--rk-n90)]" />
      </main>
    </div>
  );
}
