    ...
    <div className="absolute inset-0 z-30 flex items-center justify-center bg-[rgba(4,4,5,.62)] p-4 sm:p-10">
      ...
      >
        <div className="flex items-start justify-between gap-6">
          <h2 id="messaging-settings-title" className="text-2xl font-medium text-[var(--rk-n04)]">
        ...
        {error ? <p className="mt-4 text-[13px] text-[var(--rk-a06)]">{error}</p> : null}
        ...
        <section className="mt-8 rounded-[14px] border border-[var(--rk-n62)] bg-[var(--rk-n93)] px-4 py-4">
          ...
          {status?.identities.length ? (
            <ul className="mt-3 space-y-3">
              {status.identities.map((identity) => (
                <li
                  key={identity.id}
                  className="flex items-center justify-between gap-3 text-[14px] text-[var(--rk-n18)]"
                >
                  <span>
                    {providerLabel(identity.provider)} · {identity.address}{" "}
                    <span className="text-[12px] text-[var(--rk-n30)]">→ {identity.botName}</span>
                  </span>
                  <BuiButton
                    onClick={() =>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <select
              ...
              className="rounded-[10px] border border-[var(--rk-n56)] bg-[var(--rk-n89)] px-3 py-2 text-[13.5px] text-[var(--rk-n08)]"
            >
          ...
          {linkCode ? (
            <p className="mt-3 text-[14px] text-[var(--rk-n18)]" data-testid="messaging-link-code">
              <Trans>
                Send <span className="font-mono text-[var(--rk-n04)]">{linkCode}</span> to the line
              ...
              {channels.map((channel) => (
                ...
                >
                  <span>
                    ...
                    <span className="text-[12px] text-[var(--rk-n30)]">{channelMeta(channel)}</span>
                  </span>
              ...
              {connections.map((connection) => (
                ...
                >
                  <span>
                    ...
                    <span className="text-[12px] text-[var(--rk-n30)]">{connection.status}</span>
                  </span>
                  <span className="flex gap-2">
